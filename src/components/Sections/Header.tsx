...
const DesktopNav: FC<{navSections: SectionId[]; currentSection: SectionId | null}> = memo(
  ({navSections, currentSection}) => {
    const baseClass =
      '-m-1.5 p-1.5 rounded-md font-bold first-letter:uppercase hover:transition-colors hover:duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-800 sm:hover:text-emerald-800 text-neutral-100';
    const activeClass = classNames(baseClass, 'text-emerald-900');
    const inactiveClass = classNames(baseClass, 'text-neutral-100');
    return (
      <header className="fixed top-0 z-50 hidden w-full bg-emerald-800/50 p-4 backdrop-blur sm:block" id={headerID}>
...
const MobileNav: FC<{navSections: SectionId[]; currentSection: SectionId | null}> = memo(
  ({navSections, currentSection}) => {
    ...
    const activeClass = classNames(baseClass, 'bg-emerald-800 text-white font-bold');
        ...
        <Transition.Root as={Fragment} show={isOpen}>
          <Dialog as="div" className="fixed inset-0 z-40 flex sm:hidden" onClose={toggleOpen}>
            ...
            <Transition.Child
              ...
              <div className="relative w-4/5 bg-emerald-800">
